...
def load_all_query_data(raw_data_dir, filename, blacklist):
    ...
    df_raw = analysis_extract.load_data(
        raw_data_dir=raw_data_dir, filename=filename)
    df = transform.clean_data(df_raw, blacklist=blacklist)
    return df
...
def main():
    ...
    with Flow("etl", schedule=schedule) as flow:
        ...
        # # -- TREEMAP
        df_trend, df_top = transform_plot_data(
            df, to_uppercase=to_uppercase, top_n=top_n)
        ...
        df_analysis = transform_analysis_data(df_raw)
